...
public class AccountCollectionDistributorTest {
  ...
  // private static final BigDecimal NEG_INDIV_AMOUNT_R1 =
  // new BigDecimal("-23455.99");
  // integer (x100) versions of the above decimals
  private static final Integer INT_INDIV_AMOUNT_R1 = 2345599;
  // private static final Integer INT_NEG_INDIV_AMOUNT_R1 = -2345599;

  // amounts not-divisible by 3 (remainder 2)
  private static final BigDecimal INDIV_AMOUNT_R2 = new BigDecimal("23456.00");
  private static final BigDecimal NEG_INDIV_AMOUNT_R2 =
    new BigDecimal("-23456.00");
  // integer (x100) versions of the above decimals
  ...
  public void testAccountCollectionDistributor() {
    ...
    assertTrue("could not create distributor", distributor != null);
  ...
  /**
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#addBalance(com.poesys.accounting.dataloader.newaccounting.Account, java.math.BigDecimal)}
   * . Tests addBalance(), getBalance(), and getItemAmount().
   */
  @Test
  public void testAddBalance() {
    ...
    assertTrue("wrong item amount: " + itemAmount,
               BigDecimal.ZERO.compareTo(itemAmount) == 0);
  }

  /**
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#addBalance(com.poesys.accounting.dataloader.newaccounting.Account, java.math.BigDecimal)}
   * . Tests addBalance() with negative amount.
   */
  @Test
  public void testAddBalanceNegativeAmount() {
    AccountCollectionDistributor distributor =
      new AccountCollectionDistributor(NEG_AMOUNT);
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#addBalance(com.poesys.accounting.dataloader.newaccounting.Account, java.math.BigDecimal)}
   * . Tests addBalance() with zero amount.
   */
  @Test
  public void testAddBalanceZeroAmount() {
    ...
    BigDecimal amount = distributor.getBalance(equityAccount1);
    assertTrue("wrong amount: " + amount,
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#addBalance(com.poesys.accounting.dataloader.newaccounting.Account, java.math.BigDecimal)}
   * . Tests for addBalance given a null account throwing an exception.
   */
  @Test
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#addBalance(com.poesys.accounting.dataloader.newaccounting.Account, java.math.BigDecimal)}
   * . Tests addBalance() given a null balance throwing an exception.
   */
  @Test
  ...
  public void testIsValidBalancesEqual() {
    ...
    assertTrue("valid distributor tests as invalid (equal balances)",
               distributor.isValid());
  }
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#isValid()}
   * . Tests whether two balances that are nearly equal (differ by at most a
   * penny) is valid.
   */
  @Test
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#isValid()}
   * . Tests whether three balances that are nearly equal (differ by at most a
   * penny) is valid. Uses -1, 0, +1 differences in the test.
   */
  @Test
   ...
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#isValid()}
   * . Tests whether two balances that are nearly equal (differ by at most a
   * penny) is valid. Uses -5, 0, +5 differences in the test.
   */
  @Test
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#isValid()}
   * . Tests whether three balances that are unequal (differ by more than a
   * penny) is invalid.
   */
  @Test
  ...
  public void testIsValidNoBalances() {
    AccountCollectionDistributor distributor =
      new AccountCollectionDistributor(AMOUNT);
    assertTrue("invalid distributor tests as valid (no balances)",
               !distributor.isValid());
  }
  ...
  public void testDistributeAmount1AccountPosPos() {
    ...
    assertTrue("wrong balance " + newBalance,
               newBalance.compareTo(AMOUNT.add(AMOUNT)) == 0);
    BigDecimal newAmount = distributor.getItemAmount(equityAccount1);
    assertTrue("wrong item amount " + newAmount,
               newAmount.compareTo(AMOUNT) == 0);
  }

  /**
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution across one account for a positive amount and negative
   * balance.
   */
  @Test
  public void testDistributeAmount1AccountPosNeg() {
    ...
    BigDecimal newBalance = distributor.getBalance(equityAccount1);
    assertTrue("wrong balance " + newBalance,
               newBalance.compareTo(AMOUNT.add(NEG_AMOUNT)) == 0);
    BigDecimal newAmount = distributor.getItemAmount(equityAccount1);
    assertTrue("wrong item amount " + newAmount,
               newAmount.compareTo(AMOUNT) == 0);
  }
  ...
  public void testDistributeAmount1AccountNegPos() {
    ...
    BigDecimal newBalance = distributor.getBalance(equityAccount1);
    assertTrue("wrong balance " + newBalance,
               newBalance.compareTo(NEG_AMOUNT.add(AMOUNT)) == 0);
    BigDecimal newAmount = distributor.getItemAmount(equityAccount1);
    assertTrue("wrong item amount " + newAmount,
               newAmount.compareTo(NEG_AMOUNT) == 0);
  }

  /**
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution across one account for a negative amount and negative
   * balance.
   */
  @Test
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of positive even amount across two positive-balance
   * accounts.
   */
  @Test
  public void testDistributeAmount2EqualAccountsEvenAmountPosPos() {
    ...
    BigDecimal amountToAdd = AMOUNT.divide(new BigDecimal("2"));
    ...
    assertTrue("wrong item amount 1 " + newAmount1,
               newAmount1.compareTo(amountToAdd) == 0);
    BigDecimal newAmount2 = distributor.getItemAmount(equityAccount1);
    assertTrue("wrong item amount 2 " + newAmount2,
               newAmount2.compareTo(amountToAdd) == 0);
  }

  /**
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of positive even amount across two negative-balance
   * accounts.
   */
  @Test
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of negative even amount across two positive-balance
   * accounts.
   */
  @Test
  public void testDistributeAmount2EqualAccountsEvenAmountNegPos() {
    ...
    distributor.distributeAmount();
    BigDecimal newBalance1 = distributor.getBalance(equityAccount1);
    BigDecimal amountToAdd = NEG_AMOUNT.divide(new BigDecimal("2"));
    BigDecimal testBalance = AMOUNT.add(amountToAdd);
    assertTrue("wrong balance 1 " + newBalance1 + ", expecting " + testBalance,
               newBalance1.compareTo(testBalance) == 0);
    BigDecimal newBalance2 = distributor.getBalance(equityAccount2);
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of negative even amount across two negative-balance
   * accounts.
   */
  @Test
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of positive odd amount across two positive-balance
   * accounts.
   */
  @Test
  public void testDistributeAmount2EqualAccountsOddAmountPosPos() {
    AccountCollectionDistributor distributor =
      new AccountCollectionDistributor(ODD_AMOUNT);
    distributor.addBalance(equityAccount1, AMOUNT);
    distributor.addBalance(equityAccount2, AMOUNT);
    distributor.distributeAmount();
    BigDecimal newBalance1 = distributor.getBalance(equityAccount1);
    ...
    BigDecimal amountToAdd = new BigDecimal(intAmount).divide(CONVERTER);
    BigDecimal testBalance = AMOUNT.add(amountToAdd);
    assertTrue("wrong balance 1 " + newBalance1 + ", expecting " + testBalance,
               newBalance1.compareTo(testBalance) == 0);
    BigDecimal newBalance2 = distributor.getBalance(equityAccount2);
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of positive odd amount across two negative-balance
   * accounts.
   */
  @Test
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of negative odd amount across two positive-balance
   * accounts.
   */
  @Test
  public void testDistributeAmount2EqualAccountsOddAmountNegPos() {
    AccountCollectionDistributor distributor =
      new AccountCollectionDistributor(NEG_ODD_AMOUNT);
    distributor.addBalance(equityAccount1, AMOUNT);
    distributor.addBalance(equityAccount2, AMOUNT);
    distributor.distributeAmount();
    BigDecimal newBalance1 = distributor.getBalance(equityAccount1);
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of positive even amount across three positive-balance
   * accounts.
   */
  @Test
  public void testDistributeAmount3EqualAccountsDivisibleAmountPosPos() {
    AccountCollectionDistributor distributor =
      new AccountCollectionDistributor(DIV_AMOUNT);
    distributor.addBalance(equityAccount1, AMOUNT);
    distributor.addBalance(equityAccount2, AMOUNT);
    ...
    assertTrue("wrong item amount 2 " + newAmount3,
               newAmount3.compareTo(amountToAdd) == 0);
  }

  /**
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of positive even divisible across three
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of negative divisible amount across three
   * positive-balance accounts.
   */
  @Test
  public void testDistributeAmount3EqualAccountsDivisibleAmountNegPos() {
    AccountCollectionDistributor distributor =
      new AccountCollectionDistributor(NEG_DIV_AMOUNT);
    distributor.addBalance(equityAccount1, AMOUNT);
    distributor.addBalance(equityAccount2, AMOUNT);
    distributor.addBalance(equityAccount3, AMOUNT);
    distributor.distributeAmount();
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of positive indivisible amount across three
   * positive-balance accounts.
   */
  @Test
  public void testDistributeAmount3EqualAccountsRem2PosPos() {
    AccountCollectionDistributor distributor =
      new AccountCollectionDistributor(INDIV_AMOUNT_R2);
    distributor.addBalance(equityAccount1, AMOUNT);
    distributor.addBalance(equityAccount2, AMOUNT);
    distributor.addBalance(equityAccount3, AMOUNT);
    distributor.distributeAmount();
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeAmount()}
   * . Tests distribution of negative indivisible amount across three
   * positive-balance accounts.
   */
  @Test
   ...
   * . Tests that a single account makes no change to balances (no remainder).
   * Also tests basic balance validity. for a positive amount and a positive
   * balance.
   */
  @Test
   ...
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeRemainder()}
   * . Tests that a single account makes no change to balances (no remainder)
   * for a positive amount and a negative balance.
   */
  @Test
   ...
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeRemainder()}
   * . Tests that a single account makes no change to balances (no remainder)
   * for a negative amount and a negative balance.
   */
  @Test
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeRemainder()}
   * . Tests remainder distribution of positive, odd amount for two accounts
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeRemainder()}
   * . Tests remainder distribution of negative, odd amount for two accounts
   * with positive balances
   */
  @Test
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeRemainder()}
   * . Tests remainder distribution of positive, indivisible amount remainder 2
   * for three accounts with positive balances
   */
  @Test
  public void testDistributeRemainder3EqualAccountsRem2PosPos() {
    ...
    assertTrue("remainder not distributed properly to balances",
               (balance1.compareTo(AMOUNT) == 0
                && balance2.compareTo(expectedAmount) == 0 && balance3.compareTo(expectedAmount) == 0)
                   || (balance2.compareTo(AMOUNT) == 0
                       && balance1.compareTo(expectedAmount) == 0 && balance3.compareTo(expectedAmount) == 0)
                   || (balance3.compareTo(AMOUNT) == 0
                       && balance1.compareTo(expectedAmount) == 0 && balance2.compareTo(expectedAmount) == 0));
    ...
    assertTrue("remainder not distributed properly to items",
               (amount1.compareTo(BigDecimal.ZERO) == 0
                && amount2.compareTo(PENNY) == 0 && amount3.compareTo(PENNY) == 0)
                   || (amount2.compareTo(BigDecimal.ZERO) == 0
                       && amount1.compareTo(PENNY) == 0 && amount3.compareTo(PENNY) == 0)
                   || (amount3.compareTo(BigDecimal.ZERO) == 0
                       && amount1.compareTo(PENNY) == 0 && amount2.compareTo(PENNY) == 0));
  }
   ...
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeRemainder()}
   * . Tests remainder distribution of positive, indivisible amount remainder 2
   * for three accounts with negative balances
   */
  @Test
  public void testDistributeRemainder3EqualAccountsRem2PosNeg() {
    ...
    // + balance2 + ", " + balance3);
    assertTrue("remainder not distributed properly to balances",
               (balance1.compareTo(NEG_AMOUNT) == 0
                && balance2.compareTo(expectedAmount) == 0 && balance3.compareTo(expectedAmount) == 0)
                   || (balance2.compareTo(NEG_AMOUNT) == 0
                       && balance1.compareTo(expectedAmount) == 0 && balance3.compareTo(expectedAmount) == 0)
                   || (balance3.compareTo(NEG_AMOUNT) == 0
                       && balance1.compareTo(expectedAmount) == 0 && balance2.compareTo(expectedAmount) == 0));
    BigDecimal amount1 = distributor.getItemAmount(equityAccount1);
    BigDecimal amount2 = distributor.getItemAmount(equityAccount2);
    BigDecimal amount3 = distributor.getItemAmount(equityAccount3);
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeRemainder()}
   * . Tests remainder distribution of negative, indivisible amount remainder -2
   * for three accounts with positive balances
   */
  @Test
  public void testDistributeRemainder3EqualAccountsRem2NegPos() {
    ...
    assertTrue("remainder not distributed properly to items",
               (amount1.compareTo(BigDecimal.ZERO) == 0
                && amount2.compareTo(NEG_PENNY) == 0 && amount3.compareTo(NEG_PENNY) == 0)
                   || (amount2.compareTo(BigDecimal.ZERO) == 0
                       && amount1.compareTo(NEG_PENNY) == 0 && amount3.compareTo(NEG_PENNY) == 0)
                   || (amount3.compareTo(BigDecimal.ZERO) == 0
                       && amount1.compareTo(NEG_PENNY) == 0 && amount2.compareTo(NEG_PENNY) == 0));
  }
   ...
   * . Test balance and remainder for indivisible amount with remainder 1
   * distributed to three nearly-equal positive accounts
   */
  @Test
  public void testDistributeRemainderNearlyEqual3PosAccountsRem1() {
    AccountCollectionDistributor distributor =
      new AccountCollectionDistributor(INDIV_AMOUNT_R1);
    distributor.addBalance(equityAccount1, AMOUNT);
    distributor.addBalance(equityAccount2, AMOUNT);
    ...
    Integer newRemainder =
      distributor.distributeRemainderNearlyEqual(remainder);
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeRemainderNearlyEqual(java.lang.Integer)}
   * . Test balances and remainder for indivisible amount with remainder 2
   * distributed to three nearly-equal positive accounts
   */
  @Test
   ...
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeRemainderNearlyEqual(java.lang.Integer)}
   * . Test balance and remainder for indivisible amount with remainder 1
   * distributed to three nearly-equal negative accounts
   */
  @Test
  public void testDistributeRemainderNearlyEqual3NegAccountsRem1() {
    ...
    assertTrue("balance 1 not as expected: " + balance1 + " != "
               + NEAR_NEG_AMOUNT, balance1.compareTo(NEAR_NEG_AMOUNT) == 0);
    assertTrue("balance 2 not as expected: " + balance2 + " != " + NEG_AMOUNT,
               balance2.compareTo(NEG_AMOUNT) == 0);
    assertTrue("balance 3 not as expected: " + balance3 + " != "
               + NEAR_NEG_AMOUNT, balance3.compareTo(NEAR_NEG_AMOUNT) == 0);
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#distributeRemainderNearlyEqual(java.lang.Integer)}
   * . Test balance and remainder for indivisible amount with remainder 2
   * distributed to three nearly-equal negative accounts
   */
  @Test
  ...
  /**
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#setBalanceAndDecrementRemainder(java.lang.Integer, com.poesys.accounting.dataloader.newaccounting.Account)}
   * . Tests remainder and balance after call for positive remainder and
   * positive balances.
   */
  @Test
  public void testSetBalanceAndDecrementRemainderPosRemainderPosBalances() {
    ...
    Integer remainder =
      distributor.setBalanceAndDecrementRemainder(1, equityAccount1);
    ...
    assertTrue("item amount not set: " + amount + " != " + PENNY,
               amount.compareTo(PENNY) == 0);
  }

  /**
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#setBalanceAndDecrementRemainder(java.lang.Integer, com.poesys.accounting.dataloader.newaccounting.Account)}
   * . Tests remainder and balance after call for negative remainder and
   * positive balances.
   */
  @Test
  ...
  public void testSetBalanceAndDecrementRemainderNegRemainderNegBalances() {
    ...
    assertTrue("remainder is not zero", remainder.compareTo(0) == 0);
    BigDecimal balance = distributor.getBalance(equityAccount1);
    assertTrue("balance not set: " + balance + " != "
                   + NEG_AMOUNT.add(NEG_PENNY),
   ...
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.AccountCollectionDistributor#setBalanceAndDecrementRemainder(java.lang.Integer, com.poesys.accounting.dataloader.newaccounting.Account)}
   * . Tests whether setting an account not in the balances throws exception
   */
  @Test
  public void testSetBalanceAndDecrementRemainderInvalidAccount() {
    ...
    try {
      ...
      assertTrue("wrong exception: " + e.getMessage(),
                 e.getMessage().contains(NO_BALANCE_ERROR));
    }
  }
