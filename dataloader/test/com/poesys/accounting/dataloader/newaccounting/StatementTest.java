...
public class StatementTest {
  ...
  private static final AccountGroup CASH_GROUP = new AccountGroup(CASH_GROUP_NAME);
  private static final String INCOME_GROUP_NAME = "Earned Income";
  private static final AccountGroup INCOME_GROUP = new AccountGroup(INCOME_GROUP_NAME);
  private static final String EXPENSE_GROUP_NAME = "Household Expenses";
  private static final AccountGroup EXPENSE_GROUP = new AccountGroup(EXPENSE_GROUP_NAME);
  private static final String LIABILITY_GROUP_NAME = "Credit Accounts";
  private static final AccountGroup LIABILITY_GROUP = new AccountGroup(LIABILITY_GROUP_NAME);
  private static final String EQUITY_GROUP_NAME = "Personal Capital";
  private static final AccountGroup EQUITY_GROUP = new AccountGroup(EQUITY_GROUP_NAME);
  ...
  private final Account checkingAccount =
    new Account(CHECKING_ACCOUNT_NAME, DESCRIPTION, AccountType.ASSETS, DEBIT_DEFAULT,
                NOT_RECEIVABLE);
  private final Account expenseAccount =
    new Account(EXPENSE_ACCOUNT_NAME, DESCRIPTION, AccountType.EXPENSES, DEBIT_DEFAULT,
                NOT_RECEIVABLE);

  private final Account liabilityAccount =
    new Account(LIABILITY_ACCOUNT_NAME, DESCRIPTION, AccountType.LIABILITIES, !DEBIT_DEFAULT,
                NOT_RECEIVABLE);

  private final Account equityAccount =
    new Account(EQUITY_ACCOUNT_NAME, DESCRIPTION, AccountType.EQUITY, !DEBIT_DEFAULT,
                NOT_RECEIVABLE);

  /**
   * Test method for {@link com.poesys.accounting.dataloader.newaccounting
   * .Statement#Statement(com.poesys.accounting.dataloader.newaccounting.FiscalYear, * java.lang
   * .String, StatementType)} . Tests constructor and year, name, and type getters
   */
  @Test
  public void testStatement() {
    ...
    assertTrue("Wrong year " + statement.getYear(),
               Objects.equals(statement.getYear().getYear(), YEAR));
    assertTrue("Wrong name " + statement.getYear(), statement.getName().equals(BALANCE_SHEET_NAME));
   ...
   * Test method for {@link com.poesys.accounting.dataloader.newaccounting
   * .Statement#Statement(com.poesys.accounting.dataloader.newaccounting.FiscalYear, * java.lang
   * .String, StatementType)} . Tests null value for year, name, and type
   */
  @Test
   ...
   */
  private Statement createStatementWithRollup(FiscalYear year, Account account, AccountType type,
                                              AccountGroup group) {
    // Add account to fiscal year.
    FiscalYearAccount link = new FiscalYearAccount(year, type, group, 1, account, 1);
    year.addAccount(link);
    return new Statement(year, BALANCE_SHEET_NAME, StatementType.BALANCE_SHEET);
  }

  /**
   * Test method for {@link com.poesys.accounting.dataloader.newaccounting.Statement#getRollups()}
   * .
   */
  @Test
  public void testGetRollup() {
    FiscalYear year = new FiscalYear(YEAR);
    Statement statement =
      createStatementWithRollup(year, incomeAccount, AccountType.INCOME, INCOME_GROUP);
  ...
  /**
   * Test method for
   * {@link com.poesys.accounting.dataloader.newaccounting.Statement#getAccountBalance(Account)}
   * . Tests account balance when there is just one year.
   */
  @Test
  public void testGetAccountBalanceForBalanceSheetSingleYear() {
    FiscalYear year = new FiscalYear(YEAR);
    BigInteger nextId = createBalances(year, BigInteger.ONE);
    createFiscalYearForBalance(year, nextId);
    Statement statement = new Statement(year, BALANCE_SHEET_NAME, StatementType.BALANCE_SHEET);
    BigDecimal balance = statement.getBalance();
    for (Rollup rollup : statement.getRollups().values()) {
      BigDecimal rollupBalance = rollup.getTotal();
      Account account = rollup.getAccount();
      BigDecimal accountBalance = statement.getAccountBalance(account);
      assertTrue(
        "rollup balance different from account balance: " + rollup + ", balance " + accountBalance,
        rollupBalance.compareTo(accountBalance) == 0);
    }
  }

  /**
   * Test method for {@link com.poesys.accounting.dataloader.newaccounting.Statement#getBalance()} .
  ...
  public void testGetBalanceForBalanceSheetSingleYear() {
    ...
    BigDecimal checkBalance = new BigDecimal("25.00").setScale(2, RoundingMode.HALF_UP);
  ...
  /**
   * Test method for {@link com.poesys.accounting.dataloader.newaccounting.Statement#getBalance()} .
   * Tests balance sheet balance when there are multiple years set up sharing the accounts that list
  ...
  public void testGetBalanceForBalanceSheetMultipleYears() {
    ...
    Statement statement = new Statement(year1, BALANCE_SHEET_NAME, StatementType.BALANCE_SHEET);
    ...
    statement = new Statement(year1, BALANCE_SHEET_NAME, StatementType.BALANCE_SHEET);
    ...
    statement = new Statement(year2, BALANCE_SHEET_NAME, StatementType.BALANCE_SHEET);
    logger.info(year2.getYear() + " Transactions [\n" + statement.toDetailData() + "]");
    yearNumber++;
    FiscalYear year3 = new FiscalYear(yearNumber);
    createFiscalYearForBalance(year3, nextId);
    statement = new Statement(year3, BALANCE_SHEET_NAME, StatementType.BALANCE_SHEET);
    logger.info(year3.getYear() + " Transactions [\n" + statement.toDetailData() + "]");
    BigDecimal balance = statement.getBalance();
    for (Rollup rollup : statement.getRollups().values()) {
      switch (rollup.getAccount().getAccountType(year1)) {
        case ASSETS:
          BigDecimal checkAssetTotal = new BigDecimal("-6250.00").setScale(2, RoundingMode.HALF_UP);
          assertTrue("Asset total is not -$6,250 (debit): " + rollup.getTotal(),
                     rollup.getTotal().compareTo(checkAssetTotal) == 0);
          logger.info(rollup.getAccount().getName() + " = " + rollup.getTotal());
          break;
        case LIABILITIES:
          BigDecimal checkLiabilityTotal =
            new BigDecimal("245.00").setScale(2, RoundingMode.HALF_UP);
          assertTrue("Liability total is not $245.00 (credit): " + rollup.getTotal(),
                     rollup.getTotal().compareTo(checkLiabilityTotal) == 0);
          logger.info(rollup.getAccount().getName() + " = " + rollup.getTotal());
          break;
        case EQUITY:
          BigDecimal checkEquityTotal = new BigDecimal("6080.00").setScale(2, RoundingMode.HALF_UP);
    ...
    logger.info("Balance: " + balance);
    BigDecimal checkBalance = new BigDecimal("75.00").setScale(2, RoundingMode.HALF_UP);
  ...
  /**
   * Create a set of balances for the checking, credit card, and equity accounts.
   *
  ...
  private BigInteger createBalances(FiscalYear year, BigInteger nextId) {
    ...
    transaction = new Transaction(nextId, DESCRIPTION, date, NOT_CHECKED, BALANCE);
  ...
  /**
   * Create a set of transactions in the fiscal year that can be used to get a balance for a
   * particular kind of statement. The transactions are against the full range of accounts so the
   * set can form the basis of either a balance sheet or an income statement.
   *
   * @param year   the fiscal year in which to create transactions and accounts
   * @param nextId the next transaction id to use
   * @return the next transaction id to use
   */
  private BigInteger createFiscalYearForBalance(FiscalYear year, BigInteger nextId) {
    // Add the five accounts to the fiscal year.
    FiscalYearAccount checkingLink =
      new FiscalYearAccount(year, AccountType.ASSETS, CASH_GROUP, 1, checkingAccount, 1);
    ...
    FiscalYearAccount liabilityLink =
      new FiscalYearAccount(year, AccountType.LIABILITIES, LIABILITY_GROUP, 1, liabilityAccount, 1);
    ...
    // Income transaction
    transaction = new Transaction(nextId, DESCRIPTION, date, NOT_CHECKED, NOT_BALANCE);
    transaction.addItem(100.00D, checkingAccount, DEBIT, CHECKED);
  ...
  public void testRollupHashCodeInequality() {
    ...
    Statement statement1 =
      createStatementWithRollup(year1, incomeAccount, AccountType.INCOME, INCOME_GROUP);
    Statement statement2 =
      createStatementWithRollup(year2, incomeAccount, AccountType.INCOME, INCOME_GROUP);
    Rollup rollup1 = statement1.getRollups().get(incomeAccount);
  ...
  public void testRollupEqualsInequality() {
    ...
    Rollup rollup1 = statement1.getRollups().get(incomeAccount);
    Rollup rollup2 = statement2.getRollups().get(incomeAccount);
    assertTrue("Different rollups but equals() is true", !rollup1.equals(rollup2));
  }
  ...
  public void testStatementToString() {
    ...
    assertTrue("statement string rep failed: " + statement,
               ("Statement [year=FiscalYear [year=2017, start=2017-01-01 00:00:00.0, " +
                "end=2017-12-31 23:59:59.0], name=Balance Sheet, type=BALANCE_SHEET]").equals(
                 statement.toString()));
  }

  /**
   * Test Statement.toData() for balance sheet.
   */
  @Test
  public void testBalanceSheetToData() {
    FiscalYear year = new FiscalYear(YEAR);
    BigInteger nextId = createBalances(year, BigInteger.ONE);
    createFiscalYearForBalance(year, nextId);
    Statement statement = new Statement(year, BALANCE_SHEET_NAME, StatementType.BALANCE_SHEET);
  ...
  public void testIncomeStatementToData() {
    ...
    String data = statement.toData();
    String test =
      "Income\tEarned Income\tSalary\t100.00\n" + "Expenses\tHousehold Expenses\tEssential " +
      "Expense\t-125.00";
    assertTrue("data set incorrect for statement: " + data, test.equals(data));
  }

  /**
   * Test Statement.toDetailData() for income statement.
   */
  @Test
  public void testIncomeStatementToDetailData() {
    FiscalYear year = new FiscalYear(YEAR);
    BigInteger nextId = createBalances(year, BigInteger.ONE);
    createFiscalYearForBalance(year, nextId);
    Statement statement =
    ...
    String data = statement.toDetailData();
    assertTrue("detail data set incorrect for statement: " + data,
               ("Income\tEarned Income\tSalary\t4\t01-Jan-17\t100.00\n" + "Expenses\tHousehold " +
                "Expenses\tEssential Expense\t5\t01-Jan-17\t-50.00\n" + "Expenses\tHousehold " +
                "Expenses\tEssential Expense\t6\t01-Jan-17\t-75.00").equals(data));
  }
  ...
  public void testRollupToString() {
    ...
    assertTrue("rollup string rep failed: " + rollup,
               ("Rollup [statement=Statement [year=FiscalYear [year=2017, start=2017-01-01 " +
                "00:00:00.0, end=2017-12-31 23:59:59.0], name=Balance Sheet, " +
                "type=BALANCE_SHEET], account=Account [name=Salary, description=description, " +
                "debitDefault=false, receivable=false, years=[]], total=0.00]").equals(
                 rollup.toString()));
  }
  ...
  /**
   * Create a transaction with two items, a credit item and a debit item, with the same dollar
   * amount.
   *
   ...
   */
  private void createTransactionWithItems(BigInteger id, Double amount, Account debitAccount,
                                          Account creditAccount) {
    Transaction transaction = new Transaction(id, DESCRIPTION, DATE, NOT_CHECKED, NOT_BALANCE);
    Item debitItem = transaction.addItem(amount, debitAccount, DEBIT, NOT_CHECKED);

    assertTrue("debit item constructor failed", debitItem != null);
    Item creditItem = transaction.addItem(amount, creditAccount, CREDIT, NOT_CHECKED);
    assertTrue("credit item constructor failed", creditItem != null);
  }

  /**
   * Test Rollup.getTotal(). Test scenario with multiple items against the account, one debit and
   * one credit, and at least one item not against the account.
   */
  @Test
  public void testRollupGetTotalMultipleItems() {
    FiscalYear year = new FiscalYear(YEAR);
    Statement statement =
      createStatementWithRollup(year, checkingAccount, AccountType.INCOME, INCOME_GROUP);
    Rollup rollup = statement.getRollups().get(checkingAccount);
    // Get $100 into checking account as income.
    createTransactionWithItems(new BigInteger("1"), 100.00D, checkingAccount, incomeAccount);
    // Take $50 from checking account and expense.
    createTransactionWithItems(new BigInteger("2"), 50.00D, expenseAccount, checkingAccount);
    BigDecimal total = new BigDecimal(-50.00).setScale(2, RoundingMode.HALF_UP);
    assertTrue("rollup total for checking is wrong, should be " + total + ": " + rollup.getTotal(),
               rollup.getTotal().compareTo(total) == 0);
  }

  /**
   * Test Rollup.getTotal(). Test scenario with single item against the account, one debit.
   */
  @Test
  public void testRollupGetTotalSingleItem() {
    FiscalYear year = new FiscalYear(YEAR);
    Statement statement =
      createStatementWithRollup(year, checkingAccount, AccountType.ASSETS, CASH_GROUP);
    Rollup rollup = statement.getRollups().get(checkingAccount);
    // Get $100 into checking account as income.
    createTransactionWithItems(new BigInteger("1"), 100.00D, checkingAccount, incomeAccount);
  ...
  public void testRollupGetTotalNoTransactions() {
    ...
    Statement statement =
      createStatementWithRollup(year, checkingAccount, AccountType.ASSETS, CASH_GROUP);
    Rollup rollup = statement.getRollups().get(checkingAccount);
    BigDecimal checkTotal = BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP);
    assertTrue(
      "rollup total for checking is wrong, should be " + checkTotal + ": " + rollup.getTotal(),
