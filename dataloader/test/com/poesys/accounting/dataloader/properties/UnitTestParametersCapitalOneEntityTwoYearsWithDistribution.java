 ...
 */
public class UnitTestParametersCapitalOneEntityTwoYearsWithDistribution extends
  ...
  private static final Float CAPITAL_ACCOUNT = 300.0F;
  ...
  private static final double INCOME_AMOUNT_1 = 5000.00D;
  private static final double EXPENSE_AMOUNT_1 = 237.45D;
  private static final double DIST_AMOUNT_1 = 10.00D;

  private static final double INCOME_AMOUNT_2 = 6000.00D;
  private static final double EXPENSE_AMOUNT_2 = 574.32D;
  ...
  /** transaction date for first year as Oracle-formatted string representation */
  private static final String TRANS_DATE_1 = "01-APR-16";
  /** transaction date for second year as Oracle-formatted string representation */
  private static final String TRANS_DATE_2 = "15-AUG-17";
  ...
  private static final String REIMBURSEMENT_DATE = "20-JUL-16";
  ...
  public Reader getAccountReader(Integer year) {
    String input =
      CHECKING_ACCOUNT + DELIM + CHECKING_ACCOUNT_NAME + DELIM + CREDIT
          ...
          + LINE_RET + DIST_ACCOUNT + DELIM + DIST_ACCOUNT_NAME + DELIM + DEBIT
  ...
  public Reader getBalanceReader(Integer year) {
    ...
    String input =
          ...
          + CAPITAL_ACCOUNT + DELIM + BALANCE_DATE + DELIM + CREDIT + DELIM + CAPITAL_BALANCE_AMOUNT + LINE_RET 
          + DIST_ACCOUNT + DELIM + BALANCE_DATE + DELIM + CREDIT + DELIM + DIST_BALANCE_AMOUNT;
    // @formatter:on
    return new StringReader(input);
  }
  ...
  public Reader getTransactionReader(Integer year) {
    ...
    if (year.equals(START_YEAR)) {
      ...
      input =
            ...
            + DIST_TRANS_ID_2 + DELIM + DIST_DESC + DELIM + TRANS_DATE_1 + DELIM + FALSE;
      // @formatter:on
    } else if (year.equals(END_YEAR)) {
      // Transactions include income and expense
      // @formatter:off
      input =
              INCOME_TRANS_ID + DELIM + INCOME_DESC + DELIM + TRANS_DATE_2 + DELIM + FALSE + LINE_RET 
            ...
            + DIST_TRANS_ID_2 + DELIM + DIST_DESC + DELIM + TRANS_DATE_2 + DELIM + FALSE;
      // @formatter:on
    }
    return new StringReader(input);
  }
  ...
  public Reader getItemReader(Integer year) {
    ...
    if (year.equals(START_YEAR)) {
      ...
      input =
            ...
            + INCOME_TRANS_ID + DELIM + CHECKING_ACCOUNT + DELIM + INCOME_AMOUNT_1 + DELIM + DEBIT + DELIM + FALSE + LINE_RET 
            + INCOME_TRANS_ID + DELIM + REVENUE_ACCOUNT + DELIM + INCOME_AMOUNT_1 + DELIM + CREDIT + DELIM + FALSE + LINE_RET
            + EXPENSE_TRANS_ID + DELIM + TAX_ACCOUNT + DELIM + EXPENSE_AMOUNT_1 + DELIM + DEBIT + DELIM + FALSE + LINE_RET 
            + EXPENSE_TRANS_ID + DELIM + CREDIT_ACCOUNT + DELIM + EXPENSE_AMOUNT_1 + DELIM + CREDIT + DELIM + FALSE + LINE_RET
            + DIST_TRANS_ID_1 + DELIM + DIST_ACCOUNT + DELIM + DIST_AMOUNT_1 + DELIM + DEBIT +  DELIM + FALSE + LINE_RET
            + DIST_TRANS_ID_1 + DELIM + CASH_ACCOUNT + DELIM + DIST_AMOUNT_1 + DELIM + CREDIT + DELIM + FALSE + LINE_RET 
            + DIST_TRANS_ID_2 + DELIM + DIST_ACCOUNT + DELIM + DIST_AMOUNT_1 + DELIM + DEBIT +  DELIM + FALSE + LINE_RET 
            + DIST_TRANS_ID_2 + DELIM + CASH_ACCOUNT + DELIM + DIST_AMOUNT_1 + DELIM + CREDIT + DELIM + FALSE;
      // @formatter:on
    } else if (year.equals(END_YEAR)) {
      // Income: debit checking, credit revenue
      // Expense: debit tax expense, credit credit card
      // @formatter:off
      input =
        INCOME_TRANS_ID + DELIM + CHECKING_ACCOUNT + DELIM + INCOME_AMOUNT_2 + DELIM + DEBIT + DELIM + FALSE + LINE_RET 
            + INCOME_TRANS_ID + DELIM + REVENUE_ACCOUNT + DELIM + INCOME_AMOUNT_2 + DELIM + CREDIT + DELIM + FALSE + LINE_RET 
            + EXPENSE_TRANS_ID + DELIM + TAX_ACCOUNT + DELIM + EXPENSE_AMOUNT_2 + DELIM + DEBIT + DELIM + FALSE + LINE_RET 
            + EXPENSE_TRANS_ID + DELIM + CREDIT_ACCOUNT + DELIM + EXPENSE_AMOUNT_2 + DELIM + CREDIT + DELIM + FALSE + LINE_RET
            + DIST_TRANS_ID_1 + DELIM + DIST_ACCOUNT + DELIM + DIST_AMOUNT_2 + DELIM + DEBIT +  DELIM + FALSE + LINE_RET
            + DIST_TRANS_ID_1 + DELIM + CASH_ACCOUNT + DELIM + DIST_AMOUNT_2 + DELIM + CREDIT + DELIM + FALSE + LINE_RET 
            + DIST_TRANS_ID_2 + DELIM + DIST_ACCOUNT + DELIM + DIST_AMOUNT_2 + DELIM + DEBIT +  DELIM + FALSE + LINE_RET 
            + DIST_TRANS_ID_2 + DELIM + CASH_ACCOUNT + DELIM + DIST_AMOUNT_2 + DELIM + CREDIT + DELIM + FALSE;
      // @formatter:on
    }
    return new StringReader(input);
  }
