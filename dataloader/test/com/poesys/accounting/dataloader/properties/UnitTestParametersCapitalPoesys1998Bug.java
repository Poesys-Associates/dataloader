 ...
 */
public class UnitTestParametersCapitalPoesys1998Bug extends AbstractStatementMaintainingParameters {
  ...
  private static final String CHECKING_ACCOUNT_1_NAME = "Citicorp Checking (111222333444)";
  ...
  private static final double CAP_INFUSION_AMOUNT = CAP_INFUSION_AMOUNT_1 + CAP_INFUSION_AMOUNT_2;

  private static final double CHECKING_1_BALANCE_AMOUNT = FULL_DIST_AMOUNT - INCOME_AMOUNT;
  ...
  public Reader getAccountMapReader(Integer year) {
    String input =
      CHECKING_ACCOUNT_1 + DELIM + NEW_CHECKING_ACCOUNT_1_NAME + LINE_RET + CHECKING_ACCOUNT_2 +
      DELIM + NEW_CHECKING_ACCOUNT_2_NAME;
    return new StringReader(input);
  }
  ...
  public Reader getAccountReader(Integer year) {
    String input =
      CHECKING_ACCOUNT_1 + DELIM + CHECKING_ACCOUNT_1_NAME + DELIM + CREDIT + LINE_RET +
      CHECKING_ACCOUNT_2 + DELIM + CHECKING_ACCOUNT_2_NAME + DELIM + CREDIT + LINE_RET +
      RECEIVABLE_ACCOUNT + DELIM + RECEIVABLE_ACCOUNT_NAME + DELIM + DEBIT + LINE_RET +
      CREDIT_ACCOUNT + DELIM + CREDIT_ACCOUNT_NAME + DELIM + CREDIT + LINE_RET + CAP_ACCOUNT_1 +
      DELIM + CAP_ACCOUNT_1_NAME + DELIM + CREDIT + LINE_RET + CAP_ACCOUNT_2 + DELIM +
      CAP_ACCOUNT_2_NAME + DELIM + CREDIT + LINE_RET + DIST_ACCOUNT_1 + DELIM +
      DIST_ACCOUNT_1_NAME + DELIM + DEBIT + LINE_RET + DIST_ACCOUNT_2 + DELIM +
      DIST_ACCOUNT_2_NAME + DELIM + DEBIT + LINE_RET + REVENUE_ACCOUNT + DELIM +
      REVENUE_ACCOUNT_NAME + DELIM + CREDIT + LINE_RET + INCOME_SUMMARY_ACCOUNT + DELIM +
      INCOME_SUMMARY_ACCOUNT_NAME + DELIM + DEBIT + LINE_RET + TAX_ACCOUNT + DELIM +
      TAX_ACCOUNT_NAME + DELIM + DEBIT;
    return new StringReader(input);
  }
