...
/**
 * <p> Represents a collection of capital entities in a specific order, permitting various
 * operations to production transactions for each entity in the structure; each entity has a set of
 * specific accounts and a specific percentage ownership. Examples of standard capital structures:
 * </p> <ul> <li>individual (one entity corresponding to the individual person)</li> <li>joint (one
 * entity corresponding to a joint, multiple-person entity such as a marriage)</li> <li>sole (one
 * entity corresponding to the sole proprietor of a company)</li> <li>partnership (multiple entities
 * corresponding to the partners)</li> <li>corporation (one entity corresponding to the
 * corporation)</li> </ul> <p> The ownership percentage provides the multiplier used to allocate
 * amounts to the relevant accounts. Single-entity structures will always have this set to 100%, but
 * multiple-entity structures should validate that the entity ownership percentages sum to exactly
 * 100% using the isValid() function. </p> <p> The Income Summary account is the income account that
 * summarizes net income for transfer to the capital accounts of the capital entities. </p>
 *
...
public class CapitalStructure {
  ...
  private static final String INCOME_SUMMARY_DESCRIPTION = "Summarize income for ";
  /** description for distribution transactions */
  private static final String DISTRIBUTION_DESCRIPTION = "Transfer distribution to capital for ";
  /** description for capital account adjustment transaction */
  private static final String ADJUST_DESCRIPTION = "Adjust capital accounts to ownership";

  /** constant divisor to translate integer to monetary amount */
  private static final BigDecimal DIVISOR =
    new BigDecimal("100.00").setScale(CapitalEntity.SCALE, RoundingMode.HALF_UP);

  /** the ordered list of entities in the capital structure */
  private final List<CapitalEntity> entities = new ArrayList<>(2);
  ...
  private static final String INVALID_TRANSACTION_ERROR = "invalid income summary transaction ";
  private static final String NO_ENTITIES_ERROR = "no entities in capital structure";
  private static final String INVALID_PARAMETER_ERROR = "Invalid parameter: ";

  /**
   * Get the entities. This returns a synchronized list. If you iterate through the list, you must
   * do so in a synchronized (list) block.
   *
  ...
  /**
   * Add a list of entities to the current list. The list must have a valid ownership structure
   * after the additions.
   *
  ...
  /**
   * Is the list of entities valid as a capital structure? There must be at least one entity, and
   * the ownership percentages must sum to 1.
   *
  ...
  public boolean isValid() {
    ...
    synchronized (entities) {
      if (entities.size() > 0) {
        ...
        BigDecimal sum = BigDecimal.ZERO.setScale(CapitalEntity.SCALE, RoundingMode.HALF_UP);
        for (CapitalEntity entity : entities) {
          sum = sum.add(entity.getOwnership()).setScale(CapitalEntity.SCALE, RoundingMode.HALF_UP);
        }
        if (sum.compareTo(BigDecimal.ONE.setScale(CapitalEntity.SCALE, RoundingMode.HALF_UP)) ==
            0) {
          valid = true;
        }
      } else {
  ...
  /**
   * Get a transaction that transfers money between capital accounts to bring all the accounts to
   * their desired balances based on the capital ownership structure and tolerance of the accounting
   * system. If there is only one capital entity, the method returns null as there is never an
   * adjusting transaction for a single-entity system. If there is no change required based on the
   * current balances and ownership percentages, the method returns null.
   *
   * @param builder the builder containing the accounts, capital structure, and transactions of the
   *                accounting system
  ...
  public Transaction getCapitalAdjustmentTransaction(IBuilder builder) {
    ...
    AccountCollectionDistributor distributor = new AccountCollectionDistributor(
      BigDecimal.ZERO.setScale(CapitalEntity.SCALE, RoundingMode.HALF_UP));
    ...
    if (adjusted) {
      ...
      for (CapitalEntity entity : entities) {
        ...
        if (!item.equals(BigDecimal.ZERO.setScale(CapitalEntity.SCALE, RoundingMode.HALF_UP))) {
          boolean debit =
            item.compareTo(BigDecimal.ZERO.setScale(CapitalEntity.SCALE, RoundingMode.HALF_UP)) < 0;
   ...
   * (positive net income) or a debit (negative net income). For a credit net income, the
   * transaction debits income summary and credits the capital accounts; for a debit net income, the
   * transaction does the reverse. The allocation to multiple accounts should bring those accounts
   * to equality or near equality (at most a one cent difference between any two capital accounts).
   *
  ...
  private List<Account> getCapitalAccounts(IBuilder builder) {
    List<Account> capitalAccounts = new ArrayList<>();
    for (CapitalEntity entity : entities) {
  ...
  /**
   * Get an account distributor builder object for the fiscal year and set of entity capital
   * accounts.
   *
   ...
   */
  private AccountCollectionDistributor getDistributor(FiscalYear year, List<Account>
    capitalAccounts) {
    Statement balanceSheet = new Statement(year, BALANCE_SHEET, StatementType.BALANCE_SHEET);
    Statement incomeStatement = new Statement(year, INCOME_STMT, StatementType.INCOME_STATEMENT);
  ...
  /**
   * Build the capital account transaction with the income summary and capital items. Validate the
   * transaction. The income summary item will set the value for the Income Summary account in the
   * income statement to show the net income for the year. The capital items distribute that net
   * income to the capital accounts for the accounting entities.
   *
   * @param year            the fiscal year
   ...
   */
  private Transaction buildCapitalTransaction(FiscalYear year, IBuilder builder, List<Account>
    capitalAccounts, AccountCollectionDistributor distributor) {
    // Get the integer distributor amount, which will be positive for a net gain or
    // negative for a net loss in the income statement.
    Integer integerAmount = distributor.getAmount();
    // Convert the integer to a BigDecimal and set the scale to monetary amount.
    BigDecimal netIncome =
      new BigDecimal(integerAmount).setScale(CapitalEntity.SCALE, RoundingMode.HALF_UP);
    // Divide by the constant divisor to get the decimal amount.
    netIncome = netIncome.divide(DIVISOR, CapitalEntity.SCALE, RoundingMode.HALF_UP);
    ...
    Account incomeSummaryAccount = builder.getAccountByName(incomeSummaryAccountName);
  ...
  /**
   * <p> Get a set of transactions that help to close a fiscal year by transferring distributions or
   * draws from the distribution accounts to the corresponding capital accounts. There is always a
   * capital account, but for some accounting entities, there is no distribution account. If there
   * is such an account, the method builds a transaction for each entity that transfers the balance
   * in the account to the corresponding capital account for the entity. The debit/credit status
   * depends on the sign of the balance. The distribution account is a contra account (default
   * debit), and positive distributions debit that account; if the distribution account has a debit
   * amount, the transaction for the entity credits that account and debits the capital account
   * (removing money from capital). Distributions are not usually credits, unless there is a
   * reversal of a distribution. Also, the usual case for no distribution account is a single-entity
   * capital structure, and hence the returned list will usually be empty. </p> <p> Note that the
   * method assumes a straight transfer of amounts from the distribution accounts to the
   * corresponding capital accounts for each capital entity. If there is a requirement that the
   * capital accounts divide up by the ownership percentage, such a transfer may throw the capital
   * accounts out of whack. In this case, the caller is responsible for re-balancing the capital
   * accounts with an appropriate transaction between those accounts. This method will log a warning
   * message to highlight a possible problem in the data. </p>
   *
  ...
  public List<Transaction> getDistributionTransactions(FiscalYear year, IBuilder builder) {
    ...
    Statement stmt = new Statement(year, BALANCE_SHEET, StatementType.BALANCE_SHEET);
  ...
  public String toString() {
    return "CapitalStructure [incomeSummaryAccountName=" + incomeSummaryAccountName + ", " +
           "entities=" + entities + "]";
  }
}
