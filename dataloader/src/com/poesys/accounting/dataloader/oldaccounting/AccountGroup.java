 ...
 * exclusive; that is, the start-end intervals must not overlap. The equals()
 * method tests for equality based on name, year, and account type.
 * </p>
 * <p>
 ...
 */
public class AccountGroup extends AbstractReaderDto implements
    Comparable<AccountGroup> {
   ...
   * 
   * @return an account type or null if the interval does not correspond to a
   *         known account type
   */
  public com.poesys.accounting.dataloader.newaccounting.AccountType getAccountType() {
    ...
    if (start.compareTo(100.00F) >= 0 && end.compareTo(200.00F) < 0) {
      ...
      type =
        com.poesys.accounting.dataloader.newaccounting.AccountType.LIABILITIES;
      ...
      type =
        com.poesys.accounting.dataloader.newaccounting.AccountType.EXPENSES;
    }
    return type;
  }
  ...
  public String toString() {
    return "AccountGroup [year=" + year + ", name=" + name + ", start=" + start
           + ", end=" + end + ", orderNumber=" + orderNumber
